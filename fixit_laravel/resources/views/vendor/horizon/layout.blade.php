        ...
        <div class="d-flex align-items-center py-4 header">
            <router-link to="/" class="logo d-flex align-items-center">
                ...
                <h1 class="h4 mb-0 ms-2">
                    <strong>Мониторинг очередей</strong>{{ config('horizon.name') ? ' — ' . config('horizon.name') : '' }}
                </h1>
            </router-link>

            <div class="ms-auto">
                <scheme-toggler></scheme-toggler>
                <button class="btn btn-muted ms-2" :class="{active: autoLoadsNewEntries}" v-on:click.prevent="autoLoadNewEntries" title="Автообновление">
            ...
            <div class="col-10">
                @if ($isDownForMaintenance)
                    <div class="alert alert-warning">
                        Приложение находится в режиме обслуживания. Задачи в очереди не будут обработаны, если воркер не запущен с флагом --force.
                    </div>
                @endif
...
<script>
// Перевод динамического контента Vue
const translations = {
    'Dashboard': 'Панель управления',
    'Total Processes': 'Всего процессов',
    'Max Wait Time': 'Макс. время ожидания',
    'Max Runtime': 'Макс. время выполнения',
    'Job Runtime': 'Время выполнения задачи',
    'Throughput': 'Пропускная способность',
    'Jobs Per Minute': 'Задач в минуту',
    'Failed Jobs': 'Сбойные задачи',
    'Status': 'Статус',
    'Paused': 'На паузе',
    'Running': 'Работает',
    'Inactive': 'Неактивен',
    ...
    'No failed jobs': 'Сбойных задач нет',
    'No batches found.': 'Пакетов не найдено.',
    'Pending Jobs': 'Ожидают',
    'Completed Jobs': 'Выполнено',
    'Silenced Jobs': 'Отключённые',
    'Retry': 'Повторить',
    'Delete': 'Удалить',
    'Name': 'Имя',
    'Pushed At': 'Добавлено',
    'Completed At': 'Выполнено в',
    'Total': 'Всего',
    'Pending': 'Ожидают',
...
const observer = new MutationObserver(() => {
    const content = document.querySelector('#horizon .col-10');
    if (content) translateNode(content);
});

document.addEventListener('DOMContentLoaded', () => {
    const app = document.getElementById('horizon');
    if (app) {
