                ...
                <div class="row g-sm-4 g-3">
                    <div class="col-xl-8">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10 error-div select-dropdown">
                                <select id="blog_zones" class="select-2 form-control" id="zone" search="true" name="zone" data-placeholder="{{ __('static.zone.select-zone') }}">
                                    <option></option>
                                    @foreach ($zones as $key => $value)
                                        <option value="{{ $key }}" @if (isset($advertisement)) @if ($key == $advertisement->zone) selected @endif @endif> {{ $value }}</option>
                                    @endforeach
                                </select>
                                @error('zone')
                                    <span class="invalid-feedback d-block" role="alert">
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                function updateTotalPrice() {
                    ...
                    let adType = $('#type').val();
                    let screenPrices = {
                        home: parseFloat("{{ isset($settings['advertisement']['home_screen_price']) ? $settings['advertisement']['home_screen_price'] : '' }}"),
                        category: parseFloat("{{ isset($settings['advertisement']['category_screen_price']) ? $settings['advertisement']['category_screen_price'] : ''}}"),
                    };
                ...
                var maxImages = "{{ isset($settings['advertisement']['max_image_uploads']) ? $settings['advertisement']['max_image_uploads'] : ''}}";
