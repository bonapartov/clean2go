...
@section('content')
    ...
    @php
        $categories=Helpers::getBlogCategories();
    @endphp
                    ...
                    <form id="blogFilterForm">
                        <div class="row g-sm-4 g-3 mb-3">
                            ...
                            <div class="col-6">
                                ...
                                <select id="filterCategory" class="select-2 form-control user-dropdown Dropdown" data-placeholder="{{ __('static.service.select_categories') }}" multiple>
                                    <option class="select-placeholder" value=""></option>
                                     @foreach ($categories as $key => $option)
                                        <option value="{{ $option->id }}" image="{{ $option->getFirstMedia('image')?->getUrl() }}" data-type="{{ $option->type }}">
                                            {{ $option->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-6">
                                <label class="">{{ __('static.blog.tags') }}</label>
                                <select id="filterTags" class="select-2 form-control" data-placeholder="{{ __('static.tag.select_tags') }}" multiple>
                                    <option class="select-placeholder" value=""></option>
    ...
    <form id="exportForm" method="GET" action="{{ route('backend.blog.export') }}">
...
@push('js')
            ...
            $(document).ready(function() {
                ...
                if (urlParams.toString()) {
                    ...
                    if (urlParams.has('tags')) {
                        $('#filterTags').val(urlParams.get('tags').split(',')).trigger('change');
                    }
    ...
    <script>
        $('#exportForm').on('submit', function(e) {
            ...
            if(tags?.length) $(this).append(`<input type="hidden" name="tags" value="${tags.join(',')}">`);
            if(categories?.length) $(this).append(`<input type="hidden" name="categories" value="${categories.join(',')}">`);
