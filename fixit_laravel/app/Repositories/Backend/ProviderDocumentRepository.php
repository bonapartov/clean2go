...
use App\Exports\ProviderDocumentsFilterExport;
...
class ProviderDocumentRepository extends BaseRepository
{
    ...
    public function store($request)
    {
        DB::beginTransaction();
        try {
            $exists = $this->model->where('user_id', $request->user_id)->where('document_id', $request->document_id)->exists();
            if ($exists) {
                return back()->with('error', __('validation.document_already_exists'));
            }
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            if ($user->hasRole('provider')) {
                ...
                // ✅ Only allow update if document status is pending
                if ($userDocument->status !== 'pending') {
                    return back()->with('error', 'You can only edit documents that are pending.');
                }
                
                // ✅ Prevent duplicate document submission
                $exists = $this->model->where('user_id', $user->id)->where('document_id', $request->document_id)->where('id', '!=', $id)->exists();

                if ($exists) {
                    return back()->with('error', 'This document has already been submitted.');
                }
            }
    ...
    public function providerDocumentsFilterExport($request)
    {
        try {
            $format = $request->input('format', 'xlsx');
            ...
            if ($format == 'csv') {
                ...
                return Excel::download(new ProviderDocumentsFilterExport, 'providerDocument.csv');
            }
            return Excel::download(new providerDocumentsFilterExport, 'providerDocument.xlsx');
        } catch (Exception $e) {
            throw new Exception($e->getMessage(), $e->getCode());
        }
    }
